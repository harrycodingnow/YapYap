...
import { View, Text, StyleSheet, Platform, ScrollView, TouchableOpacity, ActivityIndicator, Alert, Pressable } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { useAuth } from '../../contexts/AuthContext';
import { useDarkMode } from '../../contexts/DarkModeContext';
import { auth, db } from '../../firebase';
import { deleteUser, signInAnonymously } from 'firebase/auth';
import { deleteDoc, collection, query, orderBy, limit, onSnapshot, where, doc, getDoc, getDocs } from 'firebase/firestore';
...
function ProfilePostCard({ post, isDarkMode }: { post: Post; isDarkMode: boolean }) {
  const { t } = useTranslation();
  return (
    <View style={[styles.postCard, isDarkMode && styles.postCardDark]}>
      <Text style={[styles.postText, isDarkMode && styles.postTextDark]}>{post.text}</Text>
      <View style={styles.postMetaRow}>
        <Ionicons name="time-outline" size={14} color={isDarkMode ? '#FDBA74' : 'black'} style={{ marginRight: 2 }} />
        <Text style={[styles.postMeta, isDarkMode && styles.postMetaDark]}>{formatTime(post.timestamp)}</Text>
        <View style={[styles.metaDivider, isDarkMode && styles.metaDividerDark]} />
        <Ionicons name="arrow-up" size={14} color={isDarkMode ? '#FDBA74' : 'black'} style={{ marginRight: 2, marginLeft: 2 }} />
        <Text style={[styles.postMeta, isDarkMode && styles.postMetaDark]}>{`${post.upvotes ?? 0} ${t('profile.upvotes')}`}</Text>
      </View>
    </View>
  );
}
...
export default function ProfileScreen() {
  ...
  const [userStats, setUserStats] = useState<UserStats>({ posts: 0, upvotes: 0, likability: 0 });
    ...
    try {
      ...
      const deleteUserPosts = async (userId: string) => {
        try {
          ...
          console.log(`🗑️ Deleted ${snapshot.docs.length} posts for user ${userId}`);
        } catch (err) {
      ...
      try {
        ...
        if (userSnap.exists()) {
          ...
          if (userData.createdAt) {
            ...
            setJoinDate(`Joined ${date.toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}`);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const likability = totalPosts > 0 ? Math.round((totalUpvotes / totalPosts) * 10) / 10 : 0;
  ...
  if (loading) {
    return (
      <View style={[styles.container, styles.loadingContainer, isDarkMode && styles.containerDark]}>
        ...
        <Text style={[styles.loadingText, isDarkMode && styles.loadingTextDark]}>{t('profile.loading')}</Text>
      </View>
    );
  }
    ...
    <View style={[styles.container, isDarkMode && styles.containerDark]}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={[styles.username, isDarkMode && styles.usernameDark]}>{t('profile.username')}</Text>
          <Text style={[styles.joinDate, isDarkMode && styles.joinDateDark]}>{joinDate || t('profile.joinedRecently')}</Text>
        </View>

        {/* Stats Card */}
        <View style={[styles.statsCard, isDarkMode && styles.statsCardDark]}>
          <View style={styles.statCol}>
            <Text style={[styles.statValue, isDarkMode && styles.statValueDark]}>{userStats.posts}</Text>
            <Text style={[styles.statLabel, isDarkMode && styles.statLabelDark]}>{t('profile.posts')}</Text>
          </View>
          <View style={[styles.statDivider, isDarkMode && styles.statDividerDark]} />
          <View style={styles.statCol}>
            <Text style={[styles.statValue, isDarkMode && styles.statValueDark]}>{userStats.upvotes}</Text>
            <Text style={[styles.statLabel, isDarkMode && styles.statLabelDark]}>{t('profile.upvotes')}</Text>
          </View>
          <View style={[styles.statDivider, isDarkMode && styles.statDividerDark]} />
          <View style={styles.statCol}>
            <Text style={[styles.statValue, isDarkMode && styles.statValueDark]}>{userStats.likability}</Text>
            <Text style={[styles.statLabel, isDarkMode && styles.statLabelDark]}>{t('profile.likability')}</Text>
          </View>
        </View>

        {/* Recent Posts */}
        <Text style={[styles.sectionTitle, isDarkMode && styles.sectionTitleDark]}>{t('profile.recentYaps')}</Text>
        {userPosts && userPosts.length > 0 ? (
          <>
            {(showAllPosts ? userPosts : userPosts.slice(0, 2)).map((post) => (
              <ProfilePostCard key={post.id} post={post} isDarkMode={isDarkMode} />
            ))}
            ...
            {userPosts.length > 2 && (
              ...
              >
                <Text style={styles.showAllText}>
                  {showAllPosts ? t('profile.showLess') : t('profile.showAll', { count: userPosts.length })}
                </Text>
              </TouchableOpacity>
            )}
          </>
        ...
        <View style={styles.actionsSection}>
          ...
          >
            <Ionicons
              ...
              style={{ marginRight: 8 }}
            />
            <Text style={[styles.actionBtnText, isDarkMode && styles.actionBtnTextDark]}>
              {language === 'en' ? t('profile.switchToChinese') : t('profile.switchToEnglish')}
            </Text>
          </TouchableOpacity>
                    ...
                    onPress: async () => {
                      try {
                        if (user?.uid) {
                          ...
                          Alert.alert(t('profile.resetComplete'), t('profile.resetCompleteBody'));
                        }
                      } catch (err) {
